...
#define LOG_INIT( format )                                              \
{                                                                             \
  boost::logging::logger_p l = boost::logging::logger::get_instance();        \
  l->add_format(format);                                                      \
}

#define LOG_ADD_OUTPUT_STREAM( sink )                                   \
{                                                                             \
  boost::logging::logger_p l = boost::logging::logger::get_instance();        \
  l->add_sink(sink);                                                          \
}

#define LOG(mask, qualifier, _trace)                                    \
{                                                                             \
  boost::logging::logger_p l = boost::logging::logger::get_instance();        \
...
#define LOG_DEBUG( trace ) \
  { LOG(LOG_LEVEL_DEBUG, boost::logging::log, trace) }

#define LOG_INFO( trace ) \
  { LOG(LOG_LEVEL_INFO, boost::logging::log, trace) }

#define LOG_WARN( trace ) \
  { LOG(LOG_LEVEL_WARN, boost::logging::log, trace) }

#define LOG_ERROR( trace ) \
  { LOG(LOG_LEVEL_ERROR, boost::logging::log, trace) }

#define LOG_EMERG( trace ) \
  { LOG(LOG_LEVEL_EMERG, boost::logging::log, trace) }

#define LOG_( mask, _trace )                                            \
  { LOG(mask, boost::logging::log, _trace) }

#define LOG_UNFORMATTED(mask, qualifier, _trace)                        \
{                                                                             \
  boost::logging::logger_p l = boost::logging::logger::get_instance();        \
  if (l->get_global_log_mask() & mask)                                        \
  {                                                                           \
...
#define LOG_SCOPE( mask, scope_object )                                 \
...
#define LOG_MAX_LINE_STR_SIZE       20    // log(2^64)
#define LOG_MASK_UP_LIMIT           65535 // 2^sizeof (mask_t) - 1
#define LOG_MASK_LEVEL_EMERG           1     // 0001
#define LOG_MASK_LEVEL_ERROR           3     // 0001
#define LOG_MASK_LEVEL_WARN            7     // 0011
#define LOG_MASK_LEVEL_INFO            15    // 0111
#define LOG_MASK_LEVEL_DEBUG           31    // 1111
...
#define LOG_L1( _trace )  LOG_( LOG_LEVEL_EMERG, _trace )
#define LOG_L2( _trace )  LOG_( LOG_LEVEL_ERROR, _trace )
#define LOG_L3( _trace )  LOG_( LOG_LEVEL_WARN,  _trace )
#define LOG_L4( _trace )  LOG_( LOG_LEVEL_INFO,  _trace )
#define LOG_L5( _trace )  LOG_( LOG_LEVEL_DEBUG, _trace )
...
//  Element classes declaration  ---------------------------------------------//
    ...
    // creation of scope_item object or the call to macro LOG_SCOPE
...
//  Sink class declaration --------------------------------------------------//
    ...
    public:
      ...
      { 
        //        m_log_mask = ((LOG_MASK_UP_LIMIT < log_mask) 
        //          ? LOG_MASK_UP_LIMIT : log_mask);
        m_log_mask = log_mask;
      }
